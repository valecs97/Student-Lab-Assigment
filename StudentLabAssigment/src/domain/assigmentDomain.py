...
class assigmentDomain:
    ...
    @staticmethod
    def readAssigment(line):
        elems = line.split(',')
        date = elems[2].split('.')
        t = assigmentDomain(int(elems[0]),elems[1],[int(date[0]),int(date[1]),int(date[2])],int(elems[3]))
        return t
    
    @staticmethod
    def writeAssigment(assigment):
        return str(assigment.assigmentId) + ',' +assigment.description + ',' + str(assigment.deadline[0])+'.'+str(assigment.deadline[1])+'.'+str(assigment.deadline[2]) + ',' + str(assigment.grade)
